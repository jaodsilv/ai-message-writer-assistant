...
describe('ChatInterface', () => {
  ...
  it('calls onEditMessage when message is edited', () => {
    render(<ChatInterface {...mockProps} />);
    ...
    expect(mockProps.onEditMessage).toHaveBeenCalledWith(expect.anything(), 'Hello edited');
  });

  it('calls onDeleteMessage when message is deleted', () => {
    render(<ChatInterface {...mockProps} />);
    const deleteButtons = screen.getAllByTitle('Delete Message');
    fireEvent.click(deleteButtons[0]);
    expect(mockProps.onDeleteMessage).toHaveBeenCalled();
  });
});
